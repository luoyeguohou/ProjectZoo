...
public class StartSeasonSys : ISystem
{
    ...
    private void StartSeason(object[] p)
    {
        ...
        if (cComp.income != 0)
        {
            Msg.Dispatch(MsgID.ActionGainCoin, new object[] { cComp.income });
            tComp.startOfSeasonInfo.Add(string.Format(Cfg.GetSTexts("ssIncome"), cComp.income));
        }
        if (EcsUtil.GetBuffNum(1) != 0)
        {
            Msg.Dispatch(MsgID.ActionGainPopularity, new object[] { EcsUtil.GetBuffNum(1) });
        ...
        {
            bool succeed = EcsUtil.RandomlyDoSth(50, () => Msg.Dispatch(MsgID.ActionGainCoin, new object[] { EcsUtil.GetBuffNum(3) }));
            if(succeed)
                tComp.startOfSeasonInfo.Add(string.Format(Cfg.GetSTexts("ssHalfChange"), EcsUtil.GetBuffNum(3)));
        }
        if (EcsUtil.GetBuffNum(4) != 0)
        {
            Msg.Dispatch(MsgID.ActionGainRandomPlotReward, new object[] { EcsUtil.GetBuffNum(4) });
        ...
        if (EcsUtil.GetBuffNum(6) > 0)
        {
            ...
            {
                Msg.Dispatch(MsgID.ActionGainPopularity, new object[] { -5 });
